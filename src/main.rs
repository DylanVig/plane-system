...
async fn main() -> anyhow::Result<()> {
    ...
    let config = cli::config::PlaneSystemConfig::read_from_path(&main_args.config)
...
async fn run_tasks(config: cli::config::PlaneSystemConfig) -> anyhow::Result<()> {
    ...
    let pixhawk_evt_rx = match config.pixhawk {
        Some(c) => {
            ...
            let evt_task = ps_pixhawk::create_task(c)?;
    ...
    if let Some(c) = config.main_camera {
        debug!("initializing camera tasks");
        let (control_task, evt_task, download_task) = ps_main_camera::create_tasks(c)?;
